...
describe("OIDC 服务", () => {
  ...
  it("未知 client 或 redirect URI 会被拒绝", () => {
    const { service } = createService();
  ...
  it("有效授权码可以交换 id_token 并且只能使用一次", async () => {
  ...
  it("错误 client secret 会被拒绝", async () => {
    const { store, service, config } = createService();
    await store.createInviteCode({ code: "JOIN", maxUses: 100 });
  ...
  it("PKCE 验证码可以生成符合 OIDC 的值", async () => {
